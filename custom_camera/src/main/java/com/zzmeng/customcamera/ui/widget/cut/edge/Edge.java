...
package com.zzmeng.customcamera.ui.widget.cut.edge;
...
public enum Edge {
    ...
    public static void offsetWidth(float offset) {
        if (getWidth() + offset * 2 <= MIN_CROP_LENGTH_PX) {
            offset = (MIN_CROP_LENGTH_PX - getWidth()) / 2;
        }
        Edge.LEFT.offset(-offset);
        Edge.RIGHT.offset(offset);
    }

    public static void offsetHeight(float offset) {
        if (getHeight() + offset * 2 <= MIN_CROP_LENGTH_PX) {
            offset = (MIN_CROP_LENGTH_PX - getHeight()) / 2;
        }
